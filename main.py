...
def get_edit_details(step):
    prompt = """The user wants to edit notes.txt. Based on this instruction, figure out exactly what text should be replaced with what.

Instruction: """ + step + """

Reply in EXACTLY this format, nothing else:
OLD: <the exact old text>
NEW: <the exact new text>"""
...
def handle_single_question(question, previous_results=""):
    ...
    needs_edit = any(word in question_lower for word in edit_keywords) and "notes.txt" not in question_lower or any(word in question_lower for word in edit_keywords)
    ...
    if "run_command" in tool:
        ...
        old_text, new_text = get_edit_details(question)
